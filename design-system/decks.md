...
| Outer padding | **tight:** 32–40 px на 1280×720 для громких cover/idea/visual-heavy слайдов. **standard:** 72–80 px для длинных КП и плотного текста. **card-protected:** tight-сетка с крупными внутренними отступами карточек |
...
- На tight-слайдах типографика держит всю композицию: H1 можно ставить почти к краю,
  но line-height должен оставаться около `0.95–1.0`, а gap между текстовыми блоками —
  намеренно компактным.
...
- Скриншоты продуктов — слегка повёрнутые карточки на graphite-фоне, без полароидной рамки.
  Можно обрезать UI-фреймы за край слайда, если это усиливает композицию и не скрывает
  смысловой фрагмент.
...
- Идеи и proof-блоки можно собирать как коллаж: карточки, стрелки, UI-скриншоты,
  упаковки/моки и логотипы становятся объектами сцены, а не ровной сеткой по умолчанию.
...
| Outer padding | весь фрейм | см. density modes в Slide canvas |
...
Используется в «режимы», «этапы», «инфраструктура», а в tight-композициях ещё и даёт
легальное пространство для воздуха: внешние gaps компактные, внутренний padding щедрый.
...
На слайдах с двумя крупными карточками держи inter-card gap плотным, а чтение спасай
внутренним padding. Если всё пространство раздать между карточками, композиция становится
обычной SaaS-сеткой.
