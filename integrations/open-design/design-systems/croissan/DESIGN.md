...
> Caring AI studio. Croissan-blue brand on near-white, with a black premium tier and graphite case-study surface. Russian-default typography on Right Grotesk + Golos Text. Pill-and-card vocabulary, polaroid photography, no emoji.
...
- **Croissan blue soft** (`#3D3DE0`): Secondary pill background (`pill.soft`) when stacking many tags — services list, wardrobe options, etc.
- **Croissan blue deep** (`#1F1FA8`): Alternative secondary pill (`pill.deep`) when soft would clash. Used sparingly.
- **Lavender chip text** (`#BCBCFF`): The text color used inside `number-box` chips on a brand-primary slide background — soft enough to read against the brand-blue gradient surface.
...
- **Paper Mute** (`white at 55%`): Tertiary text on dark — captions, supporting labels.
- **Line** (`white at 18%`): The 1px divider on dark surfaces (between stage rows in a pricing breakdown).
...
The only gradient that appears in the system is the **number-box surface** on Croissan-blue slides:
...
- **Geist Mono** (mono): OFL, bundled at [`assets/fonts/geist-mono/`](../../../../assets/fonts/geist-mono/). CSS variable `--font-geist-mono`. Code blocks, technical readouts.
...
| Role | Size | Weight | Line Height | Letter Spacing | Notes |
|------|------|--------|-------------|----------------|-------|
| H1 standard | 90 px | 700 (Bold) | 1.0 | −1.8 px (≈ −2%) | Cover, agenda, content slides. White on Croissan blue. |
...
Three filled variants only (no outline ring, no white «light» pill on brand-blue slide backgrounds). Same shape (`rounded-[999px]`, `padding ≈ .85em 1.25em`, Golos Medium ≈ 1vw, `letter-spacing: -.005em`), differ only in fill:

| Variant | Surface | Text | When |
|---------|---------|------|------|
| `pill.dark`    | `#0E0E14` (ink) | white | Accent tags, contacts on brand slides |
| `pill.soft`    | `#3D3DE0` (croissan-blue-soft) | white | Service / tag clusters |
| `pill.deep`    | `#1F1FA8` (croissan-blue-deep) | white | Alternative when `soft` would clash |

If you want a fourth pill variant, **don't** — tune padding or weight inside these three.
...
| Variant | Surface | Text | When |
|---------|---------|------|------|
| `card` (default) | `#0E0E14` or `#1A1A22` (ink-soft) | white | Standard / alternate tier |
| `card.featured` | white | `#2727CA` | Recommended option, "starred" mode, featured tier |

Inside each card, fold labels into `.ttl` (no separate uppercase eyebrow row):

- `.ttl` — Bold `~1.7vw`, `letter-spacing:-.012em` — the title (may include stage/mode prefix).
...
### Number-box / chip (brand / Croissan-blue slides only)
...
| Slide cards | No shadow. Solid fill only (ink / ink-soft / featured white). |
...
- ❌ **Extra pill variants** beyond `dark` / `soft` / `deep`, or **outline/light pills** on brand slides.
- ❌ **Outline cards** or more than **two** slide-card treatments (`card`, `card.featured`).
